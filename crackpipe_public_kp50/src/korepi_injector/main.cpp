...
int main(int argc, char* argv[]) {
    ...
    if (!val) {
        val = R"({{"path":"{}","envStr":"Crack","envStrP":"Crack","role":"31","discordId":"0","secret_extra":"frAQBc8W","isEnterDoorLoad":"true"}})";
        ini.SetValue("inject", "secrets", val);
    }
    ini.SaveFile("cfg.ini");

    std::string inject_secrets = std::string(val).c_str();
    std::string filename = (argc == 2 ? argv[1] : "korepi50.dll");
    std::filesystem::path currentDllPath = std::filesystem::current_path() / filename;
    std::string path = std::filesystem::current_path().string().c_str();
    ...
    LOG("Injecting dll.dll\n");
    ManualMapDLL_(hProcess, (std::filesystem::current_path() / "dll.dll").string(), "");
    Sleep(500);
    LOG("Injecting korepi.dll\n");
    ManualMapDLL_(hProcess, currentDllPath.string(), i_want_to_be_cool_when_i_grow_up);
    Sleep(1500);
    LOG("Resume Genshin Thread\n");
    ResumeThread(hThread);
    LOG("Inject success, exit in 5 seconds...\n");
    Sleep(5000);
    ExitProcess(0);
}