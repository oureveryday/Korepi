...
int main(int argc, char* argv[]) {
    ...
    if (!val) {
        val = R"({{"path":"{}","discordId":"80692","secret_extra":"frAQBc8W","dataId":"44262:mokPVuACUwR5Qw==","hwid":"---------Hi-Korepi-Devs---------:---------Hi-Korepi-Devs---------"}})";
        ini.SetValue("inject", "secrets", val);
    }
    ini.SaveFile("cfg.ini");

    std::string inject_secrets = std::string(val).c_str();
    std::string filename = (argc == 2 ? argv[1] : "korepi46.dll");
